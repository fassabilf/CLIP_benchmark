...
#SBATCH -t 04:00:00
#SBATCH -A lt200394
#SBATCH -J clipkd_mammoth_only_v6_enc_probe
#SBATCH -a 0-51%10
...
# Phase A: encode clipkd_mammoth_only_v6 checkpoints (e8/e16/e24/e32) over the same SEA training
...
# After all tasks finish, run runs/aj_clipkd_mammoth_only_v6_retrieval_probe.sh (Phase B) to get
# cos/R@1.
...
TAGS=(clipkd_mammoth_only_v6_e8 clipkd_mammoth_only_v6_e16 clipkd_mammoth_only_v6_e24 clipkd_mammoth_only_v6_e32)
DSENTRIES=(
  "bloom:" \
...
case "$TAG" in
  clipkd_mammoth_only_v6_e8)  CKPT="$CLIPKD_MAMMOTH_ONLY_V6_E8_CKPT" ;;
  clipkd_mammoth_only_v6_e16) CKPT="$CLIPKD_MAMMOTH_ONLY_V6_E16_CKPT" ;;
  clipkd_mammoth_only_v6_e24) CKPT="$CLIPKD_MAMMOTH_ONLY_V6_E24_CKPT" ;;
  clipkd_mammoth_only_v6_e32) CKPT="$CLIPKD_MAMMOTH_ONLY_V6_E32_CKPT" ;;
  *) echo "bad tag $TAG"; exit 1 ;;
esac

if [[ ! -f "$CKPT" ]]; then
  echo "ERROR: checkpoint not found at $CKPT"
  echo "Check $CLIPKD_MAMMOTH_ONLY_V6_DIR for the actual filenames."
  exit 1
fi
