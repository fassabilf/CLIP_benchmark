#!/bin/bash
# Phase B: run retrieval_from_emb_safe.py for ALL epochs of all 4 models in Asenthil's
# KD-loss ablation (ckd/fdonly/icl/nokd).
# Run on login/transfer node after ak_clipkd_ablation4_enc_probe.sh finishes.
#
# Usage:
#   bash runs/ak_clipkd_ablation4_retrieval_probe.sh
# Monitor:
#   tail -f nohup_retrieval_clipkd_ablation4.out
...
TAGS=()
for m in clipkd_ckd_v1 clipkd_fdonly_v1 clipkd_icl_v1 clipkd_nokd_v1; do
  for e in 8 16 24 32; do
    TAGS+=("${m}_e${e}")
  done
done

DRIVER_LOG="$(pwd)/nohup_retrieval_clipkd_ablation4.out"
