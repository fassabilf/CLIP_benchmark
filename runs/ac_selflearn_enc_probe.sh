...
# Phase A: encode selflearn_mammoth_v1 checkpoints (e8/e16/e24/e32) over SEA training
# shards (bloom + cgoe×6 + wit×6 = 13 entries per epoch).
# 4 epochs × 13 entries = 52 tasks, <=10 concurrent.
# Selflearn = no KD (contrastive loss only), trained on SEA+CC12M+Bloom, random init.
# Encoder auto-skips existing .npz -> safe to resubmit.
# After all tasks finish, run ac_selflearn_retrieval_probe.sh (Phase B) to get cos/R@1.
...
case "$TAG" in
  selflearn_mammoth_v1_e8)  CKPT="$SELFLEARN_MAMMOTH_V1_E8_CKPT"  ;;
  selflearn_mammoth_v1_e16) CKPT="$SELFLEARN_MAMMOTH_V1_E16_CKPT" ;;
  selflearn_mammoth_v1_e24) CKPT="$SELFLEARN_MAMMOTH_V1_E24_CKPT" ;;
  selflearn_mammoth_v1_e32) CKPT="$SELFLEARN_MAMMOTH_V1_E32_CKPT" ;;
  *) echo "bad tag $TAG"; exit 1 ;;
esac

if [[ ! -f "$CKPT" ]]; then
  echo "ERROR: checkpoint not found at $CKPT"
  echo "Check $SELFLEARN_MAMMOTH_V1_DIR for the actual filenames."
  exit 1
fi
