...
case "$TAG" in
  ckdonly_v1_e8)  CKPT="$CKDONLY_V1_E8_CKPT" ;;
  ckdonly_v1_e16) CKPT="$CKDONLY_V1_E16_CKPT" ;;
  ckdonly_v1_e24) CKPT="$CKDONLY_V1_E24_CKPT" ;;
  ckdonly_v1_e32) CKPT="$CKDONLY_V1_E32_CKPT" ;;
  *) echo "bad tag $TAG"; exit 1 ;;
esac

if [[ ! -f "$CKPT" ]]; then
  echo "ERROR: checkpoint not found at $CKPT"
  echo "Check $CKDONLY_V1_DIR for the actual filenames."
  exit 1
fi
