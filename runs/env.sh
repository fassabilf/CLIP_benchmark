...
# Habibi ckdonly_v1: ViT-T-16 <- MetaCLIP2-ViT-B-16-worldwide (clipkd distill only, no FD/ICL),
# same student config as its mammoth-family siblings (SigLIP2 HFTokenizer, vocab=256000,
# eval in mteb_env2 / open_clip_edit, NOT mc2_eval_env's CLIP-BPE 49408 config — confirmed by
# token_embedding.weight shape [256000, 384] on checkpoint load). Trained on CC12M + full SEA
# blend (CG-OE-filt×6 + WIT-hf-base×6 + Bloom) + mammoth_vl_sea, 32ep.
CKDONLY_V1_DIR="/project/lt200394-thllmV/mkd-exp/open_clip/experiments/ckdonly_v1/ckdonly_ViT-T-16_metaclip2_v1/checkpoints"
CKDONLY_V1_E8_CKPT="$CKDONLY_V1_DIR/epoch_8.pt"
CKDONLY_V1_E16_CKPT="$CKDONLY_V1_DIR/epoch_16.pt"
